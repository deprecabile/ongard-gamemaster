...
public class CustomExceptionInterceptor {
  ...
  public ResponseEntity<ApiError> handleValidationException(MethodArgumentNotValidException ex) {
    List<Message> messages = ex.getBindingResult().getFieldErrors().stream().map(error -> Message.builder().level(Message.Level.ERROR).code("AU_400_00").message(error.getField() + ": " + error.getDefaultMessage()).build()).toList();
