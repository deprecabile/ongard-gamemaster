...
public class ChatClient {
  ...
  public ChatClient(@Value( "${ondgard.chat.url}" ) String chatUrl) {
    this.restClient = RestClient.builder().baseUrl(chatUrl).build();
  }
  ...
  public void createUser(UUID userHash, String username) {
    ...
    try{
      restClient.post().uri("/api/internal/user").contentType(MediaType.APPLICATION_JSON).body(Map.of("userHash", userHash, "username", username)).retrieve().toBodilessEntity();
