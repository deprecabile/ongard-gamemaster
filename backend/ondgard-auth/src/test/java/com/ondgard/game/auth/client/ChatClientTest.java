...
class ChatClientTest {
  ...
  void createUser_callsChatServiceSuccessfully() {
    ...
    verify(requestBodyUriSpec).uri(eq("/api/internal/user"), any(Object[].class));
    verify(responseSpec).toBodilessEntity();
  }

  @Test
  void createUser_doesNotThrowOnSuccess() {
    assertThatCode(() -> chatClient.createUser(UUID.randomUUID(), "player1")).doesNotThrowAnyException();
  }
  ...
  void createUser_wrapsRestClientExceptionInAppException() {
    ...
    assertThatThrownBy(() -> chatClient.createUser(UUID.randomUUID(), "player1")).isInstanceOf(AppException.class).hasMessageContaining("Failed to sync user with game service");
  }
