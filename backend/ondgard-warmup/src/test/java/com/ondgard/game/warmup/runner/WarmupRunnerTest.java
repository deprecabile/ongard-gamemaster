...
class WarmupRunnerTest {
  ...
  void run_completesAllPhases_whenEverythingSucceeds() throws Exception {
    ...
    verify(redisTemplate).convertAndSend("ondgard:rag:warmup:completed", "ok");
  }

  @Test
  void run_publishesRagWarmupTopic_whenAllPhasesSucceed() throws Exception {
    when(ollamaClient.isReady(anyString())).thenReturn(true);
  ...
  @Test
  void run_completesSuccessfully_whenGpuIsUnreachable() throws Exception {
    when(ollamaClient.isReady(GPU_URL)).thenReturn(false);
    when(ollamaClient.isReady(CPU_URL)).thenReturn(true);

    runner.run();

    verify(ollamaClient, never()).warmUpGenerate(eq(GPU_URL), anyString(), anyInt());
    verify(ollamaClient).warmUpGenerate(CPU_URL, CPU_MODEL, TIMEOUT);
    verify(ollamaClient).warmUpEmbed(CPU_URL, CPU_EMBED, TIMEOUT);
    verify(redisTemplate).convertAndSend("ondgard:rag:warmup:completed", "ok");
    verify(valueOps).set("ondgard:ollama:ready", "ok");
  ...
  @Test
  void run_completesSuccessfully_whenCpuIsUnreachable() throws Exception {
    when(ollamaClient.isReady(GPU_URL)).thenReturn(true);
    when(ollamaClient.isReady(CPU_URL)).thenReturn(false);

    runner.run();

    verify(ollamaClient).warmUpGenerate(GPU_URL, GPU_MODEL, TIMEOUT);
    verify(ollamaClient, never()).warmUpGenerate(eq(CPU_URL), anyString(), anyInt());
    verify(ollamaClient, never()).warmUpEmbed(anyString(), anyString(), anyInt());
    verify(redisTemplate).convertAndSend("ondgard:rag:warmup:completed", "ok");
    verify(valueOps).set("ondgard:ollama:ready", "ok");
  }

  @Test
  void run_completesSuccessfully_whenBothAreUnreachable() throws Exception {
    when(ollamaClient.isReady(anyString())).thenReturn(false);

    runner.run();

    verify(ollamaClient, never()).warmUpGenerate(anyString(), anyString(), anyInt());
    verify(ollamaClient, never()).warmUpEmbed(anyString(), anyString(), anyInt());
    verify(redisTemplate).convertAndSend("ondgard:rag:warmup:completed", "ok");
    verify(valueOps).set("ondgard:ollama:ready", "ok");
  }
  ...
  @Test
  void run_skipsWarmup_whenModelNotFound() throws Exception {
    when(ollamaClient.isReady(anyString())).thenReturn(true);

    doThrow(new OllamaHealthClient.ModelNotFoundException("not found"))
        .when(ollamaClient).warmUpGenerate(GPU_URL, GPU_MODEL, TIMEOUT);

    runner.run();

    verify(ollamaClient, times(1)).warmUpGenerate(GPU_URL, GPU_MODEL, TIMEOUT);
    verify(valueOps).set("ondgard:ollama:ready", "ok");
  }
