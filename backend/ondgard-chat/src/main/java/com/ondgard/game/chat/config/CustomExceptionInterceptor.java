...
import com.ondgard.game.exception.*;
...
public class CustomExceptionInterceptor {
  ...
  public ResponseEntity<ApiError> handleTokenLimitExceededException(TokenLimitExceededException ex) {
    return ResponseEntity.status(HttpStatus.TOO_MANY_REQUESTS).body(ex.getApiError());
  }

  @ExceptionHandler( ServiceUnavailableException.class )
  public ResponseEntity<ApiError> handleServiceUnavailableException(ServiceUnavailableException ex) {
    return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE).body(ex.getApiError());
  }
