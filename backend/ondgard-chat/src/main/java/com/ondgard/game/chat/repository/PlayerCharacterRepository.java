...
public interface PlayerCharacterRepository extends JpaRepository<PlayerCharacterEntity, Long> {
  ...
  @Query( """
      SELECT new com.ondgard.game.chat.repository.projection.PlayerCharacterProjection(
  ...
  Optional<PlayerCharacterProjection> findByCharacterHashAndUserHash(
      @Param( "characterHash" ) String characterHash, @Param( "userHash" ) UUID userHash);
  ...
  @Query( value = """
      ...
      """, nativeQuery = true )
  void insertCharacter(@Param( "userHash" ) UUID userHash, @Param( "raceCode" ) String raceCode, @Param( "characterHash" ) String characterHash, @Param( "name" ) String name, @Param( "description" ) String description);

  @Modifying
  @Query( "DELETE FROM PlayerCharacterEntity pc WHERE pc.characterHash = :characterHash AND pc.user.userHash = :userHash" )
  int deleteByCharacterHashAndUserHash(@Param( "characterHash" ) String characterHash, @Param( "userHash" ) UUID userHash);
}
