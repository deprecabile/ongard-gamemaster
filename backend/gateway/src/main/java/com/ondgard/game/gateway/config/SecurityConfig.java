...
public class SecurityConfig {
  ...
  public SecurityWebFilterChain publicSecurityFilterChain(ServerHttpSecurity http) {
    return http.securityMatcher(new OrServerWebExchangeMatcher(
        ...
        new PathPatternParserServerWebExchangeMatcher("/api/chat/health")
    )).csrf(ServerHttpSecurity.CsrfSpec::disable).authorizeExchange(exchanges -> exchanges.anyExchange().permitAll()).build();
  }

  @Bean
  @Order( 2 )
  public SecurityWebFilterChain protectedSecurityFilterChain(ServerHttpSecurity http) {
    return http.csrf(ServerHttpSecurity.CsrfSpec::disable).authorizeExchange(exchanges -> exchanges.pathMatchers("/api/chat/user").denyAll().anyExchange().authenticated()
